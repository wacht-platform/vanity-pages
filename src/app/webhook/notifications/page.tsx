				...
				<div className="space-y-3">
					<div className="flex w-full items-center gap-2">
						<input
							...
							className="h-9 w-full rounded-md border border-border/60 bg-card px-3 text-sm outline-none transition-colors focus:border-ring"
						/>
						<Button
							type="button"
							variant="secondary"
					...
					<div className="flex min-h-10 flex-wrap items-center gap-2 rounded-md border border-border/60 bg-card p-2">
						{emails.length === 0 ? (
							...
							emails.map((email) => (
								<div
									key={email}
									className="inline-flex items-center gap-1 rounded-md border border-border/60 bg-secondary/50 px-2 py-1 text-xs text-foreground"
								>
									<span>{email}</span>
									<button
										type="button"
