...
export function getFilesystemPaneBounds(containerWidth: number) {
    const minWidth = Math.min(
        420,
        Math.max(300, Math.floor(containerWidth * 0.32)),
    );
    const maxWidth = Math.max(
        minWidth,
        Math.min(920, Math.floor(containerWidth * 0.68)),
    );
...
export function clampFilesystemPaneWidth(
    width: number,
...
export function isNoteMessage(content: ConversationContent): boolean {
    return content.type === "system_decision" && content.step === "note";
}
...
export function getResponseAttachments(
...
export function messageDisplayKind(
    content: ConversationContent,
): "user" | "agent" | "system" {
    switch (content.type) {
        case "user_message":
