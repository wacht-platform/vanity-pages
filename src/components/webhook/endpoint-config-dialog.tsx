...
export function EndpointConfigDialog({
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-[1080px] p-0 bg-popover border border-border shadow-lg rounded-[12px] overflow-hidden flex flex-col h-[85vh]">
					...
					<div className="grid grid-cols-1 md:grid-cols-12 gap-8 h-full">
						<div className="md:col-span-4 space-y-6 overflow-y-auto custom-scrollbar pr-2">
							<div className="space-y-2">
								<Label htmlFor="url" className="font-mono text-[10px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Endpoint URL</Label>
								<Input id="url" placeholder="https://example.com/webhook" value={url} onChange={(e) => setUrl(e.target.value)} className="bg-background border-border-strong h-9 font-normal text-sm focus-visible:ring-primary/20" />
							</div>
							<div className="space-y-2">
								<Label htmlFor="description" className="font-mono text-[10px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Description (optional)</Label>
								<textarea id="description" value={descriptionText} onChange={(e) => setDescriptionText(e.target.value)} className="flex min-h-[80px] w-full rounded-[6px] border border-border-strong bg-background px-3 py-2 text-sm font-normal ring-offset-background placeholder:text-muted-foreground/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-50 resize-none transition-all" />
							</div>
									...
									<div className="mt-4 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
										<div className="space-y-3">
											<div className="flex items-center justify-between">
												<Label className="font-mono text-[10px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Custom Headers</Label>
											...
											<div className="space-y-2">
												{headers.length === 0 ? <p className="text-xs font-normal text-muted-foreground/50 italic py-1">No custom headers defined.</p> : headers.map((header, index) => (
													<div key={index} className="flex items-center gap-2 group animate-in fade-in slide-in-from-left-1 duration-200">
														<Input placeholder="Key" value={header.key} onChange={(e) => setHeaders((prev) => prev.map((h, i) => (i === index ? { ...h, key: e.target.value } : h)))} className="h-8 text-[11px] font-normal bg-secondary border-border-strong focus:ring-primary/10" />
														<Input placeholder="Value" value={header.value} onChange={(e) => setHeaders((prev) => prev.map((h, i) => (i === index ? { ...h, value: e.target.value } : h)))} className="h-8 text-[11px] font-normal bg-secondary border-border-strong focus:ring-primary/10" />
											...
											<div className="mt-3 animate-in fade-in zoom-in-95 duration-200">
												<div className="flex items-center gap-3">
													<div className="flex-1 space-y-1.5">
														...
														<Input type="number" value={rateLimitMaxRequests} onChange={(e) => setRateLimitMaxRequests(e.target.value)} className="h-8 text-xs font-normal bg-background border-border-strong" />
													</div>
													<div className="flex-1 space-y-1.5">
														...
														<Select value={rateLimitDurationMs} onValueChange={setRateLimitDurationMs}>
															<SelectTrigger className="h-8 text-xs font-normal bg-background border-border-strong"><SelectValue /></SelectTrigger>
															<SelectContent>
						...
						<div className="md:col-span-8 flex flex-col space-y-4 min-h-0">
							<div className="flex items-center justify-between">
								<div className="space-y-1">
									<Label className="font-mono text-[10px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Event Subscriptions</Label>
							...
							<div className="relative">
								...
								<Input placeholder="Search event types, schemas, descriptions..." value={eventSearch} onChange={(e) => setEventSearch(e.target.value)} className="pl-10 h-[30px] text-sm font-normal bg-secondary border-border-strong focus-visible:ring-primary/20" />
							</div>
							<div className="flex-1 border border-border rounded-[10px] bg-secondary overflow-hidden flex flex-col min-h-0">
									...
									{eventsLoading ? (
										...
										<div key={group} className="bg-background">
											<div className="px-4 py-2 bg-secondary border-b border-border">
												<span className="font-mono text-[10px] font-medium uppercase tracking-[0.06em] text-muted-foreground">{group}</span>
											</div>
											<div className="divide-y divide-border">
												{groupEvents.map((event) => {
													...
													return (
														<div key={event.event_name} className={cn("flex flex-col transition-colors", isSelected ? "bg-secondary" : "hover:bg-secondary")}>
															<div className="flex items-start gap-4 px-4 py-3 cursor-pointer group/item" onClick={() => toggleEvent(event.event_name)}>
																<div className="pt-1">
																	<input type="checkbox" checked={isSelected} onChange={() => { }} className="w-4 h-4 rounded border-border-strong text-primary focus:ring-primary/20 cursor-pointer transition-all" />
																</div>
																<div className="flex-1 min-w-0">
																	<div className="flex items-center justify-between gap-4">
																		...
																		<button onClick={(e) => toggleEventExpand(event.event_name, e)} className="flex items-center gap-1.5 text-[11px] font-normal text-muted-foreground hover:text-foreground transition-colors py-0.5 px-2 rounded-[6px] hover:bg-secondary border border-transparent hover:border-border">
															...
															{isExpanded ? (
																<div className="px-4 pb-5 pt-1 animate-in slide-in-from-top-1 duration-200">
																	<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-4 rounded-[10px] bg-card border border-border">
																		<div className="space-y-3 min-w-0">
																			<div className="flex items-center justify-between border-b border-border pb-2">
																				<h4 className="font-mono text-[10px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Payload Schema</h4>
																				{event.schema ? <span className="inline-flex items-center rounded-[4px] bg-primary/10 px-1.5 text-[10px] font-medium uppercase tracking-[0.06em] text-primary">JSON Schema</span> : null}
																			</div>
																		...
																		<div className="space-y-3 min-w-0">
																			<div className="flex items-center justify-between border-b border-border pb-2">
																				<h4 className="font-mono text-[10px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Example JSON</h4>
																			...
																			<div className="p-3 bg-card rounded-[10px] border border-border overflow-hidden">
																	...
																	{isSelected ? (
																		<div className="mt-3 space-y-2">
																			<Label className="font-mono text-[10px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Filter Rules (JSON, optional)</Label>
																			<textarea value={eventFilterJson[event.event_name] || ""} onChange={(e) => setEventFilterJson((prev) => ({ ...prev, [event.event_name]: e.target.value }))} placeholder='{"user.id":{"$eq":"123"}}' className="min-h-[96px] w-full rounded-[6px] border border-border-strong bg-background px-3 py-2 text-xs font-normal text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20" />
				...
				<DialogFooter className="p-6 border-t border-border bg-secondary">
					<div className="flex flex-col sm:flex-row justify-between w-full items-center gap-4">
						<div className="flex-1">
							{error ? (
								<div className="flex items-center gap-2 text-error bg-error-soft px-3 py-1.5 rounded-md border border-error/20">
									<p className="text-[11px] font-normal">{error}</p>
								</div>
							) : null}
						</div>
						<div className="flex items-center gap-3">
							<Button variant="ghost" onClick={() => onOpenChange(false)} disabled={submitting} className="h-9 px-5 text-xs font-normal text-muted-foreground hover:text-foreground hover:bg-secondary">Cancel</Button>
